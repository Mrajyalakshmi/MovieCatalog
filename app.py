...
def show_genre(genre_id):
    ...
    movies = session.query(Movie).filter_by(
     genre_id=genre.id).order_by(asc(Movie.name))
    return render_template(
        'genre.html',
        genres=genres,
        genre=genre,
...
def gconnect():
    ...
    flash(
        'You are now logged in as {name}'.format(
...
def genre_movies_json(genre_id):
    """JSON data for genres."""
    movie_list = db.get_movies(genre_id)
    genre = db.get_genre(genre_id)
    return jsonify(
        Genre=genre.name,
        Movies=[movie.serialize for movie in movie_list])
